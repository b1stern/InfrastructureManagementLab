...
1) Review this step only, this step was done for you.  In order to use the Console Access capability, servers must be onboarded.  The onboarding process has already been performed for one of the servers in the environment.  See steps a) and b) below to see what the onboarding process looks like.
...
2) You can search for a server by name, or you can click on "Onboard only" checkbox to filter the list of servers shown.  Or, use a combination of both filters for a large environment.  

![](images/2021-05-03-16-36-01.png)

1) On the Console access page, you can open an SSH-over-HTTPS session by clicking onto the link under the SSH column to access the terminal for the particular server  from IBM Cloud Pak console (the name and IP address on the pictures may be different then what you see in the environment - but there will be one server onboarded and available).
