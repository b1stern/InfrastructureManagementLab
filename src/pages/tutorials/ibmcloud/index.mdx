...
### Verifying new instances in Infrastructure Management view
  ...
  Go to the browser tab where you have Infrastructure Management opened and select Instances for cloud provider "IBM Cloud US-East" (Compute -> Cloud -> Instances). On the list, you should see new VM instance created by you in the previous step (HINT: the refresh is done periodically, so it may take few minutes for your virtual machines to show up). Click any of them to see details.
