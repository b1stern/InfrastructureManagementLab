...
### Exploring the service library
  ...
  New version of the service is opened in Service editor. Go to the **Composition** tab (1), unfold the available ansible tasks on the left (2) and drag-and-drop **Nginx Configuration** task next to the Ansible inventory task in the flow (3). 
  ...
  Provide the required parameters. For **service name** (1) provide a unique name (for example your user id - this will help you identify your instance among all VMs provisioned by other students). For **Public ssh key** (3) you have to provide a public ssh key to be injected into the Virtual machines.
  
  You can use existing public ssh key or generate a new one. If you are using Windows as your workstation you can use [PuTTYgen](https://www.ssh.com/academy/ssh/putty/windows/puttygen) to generate new key pair. For Linux and MacOS open the Terminal window and run the following command (hit ENTER twice when prompted for passphrase):
  ...
  If you generated new one, list the content of **student.pub** file (cat student.pub) and copy the key to the provisioning dialog in the **Public SSH key** field. You can change **Environment** type (4) - that will be dynamically mapped to the filters in the Infrastructure management UI. Finally hit **Deploy** button (5).
...
### Verifying new instances in Infrastructure Management view
  ...
  Go to the browser tab where you have Infrastructure Management opened and select Instances for cloud provider "IBM Cloud US-East" (Compute -> Cloud -> Instances). On the list, you should see two instances created by you in the previous step (HINT: the refresh is done periodically, so it may take few minutes for your virtual machines to show up). Click any of them to see details.
