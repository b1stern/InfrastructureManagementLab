...
7) Click the 3 dots on the right side and then select "Duplicate"
  ![](images/DuplicateService.png)


8) A dialog will open.  At the bottom of the dialog you will see a name that was automatically populated with "Copy of VMware Single VM with DHCP".   Note:  you may have to scroll down within the dialog in order to see the "Name" field.  Change the name to a name that begins with your username. For example, "student37 VMware service".  Then, click the "Duplicate" button.
